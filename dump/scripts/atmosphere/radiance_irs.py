...
# ---save the soundings selected from this dwell
    ...
    satellite_azimuth_angle = np.concatenate((satellite_azimuth_angle, source[imax, jmax]))
    source = loca.variables["satellite_zenith_angle"][:][:]
    satellite_zenith_angle = np.concatenate((satellite_zenith_angle, source[imax, jmax]))
    source = loca.variables["solar_azimuth_angle"][:][:]
    solar_azimuth_angle = np.concatenate((solar_azimuth_angle, source[imax, jmax]))
    source = loca.variables["solar_zenith_angle"][:][:]
    solar_zenith_angle = np.concatenate((solar_zenith_angle, source[imax, jmax]))
    ...
    mwir_global_pcr_scores = np.concatenate((mwir_global_pcr_scores, source[imax, jmax]))
    source = mwva.variables["global_pcrs_quality"][:][:]
    mwir_global_pcrs_quality = np.concatenate((mwir_global_pcrs_quality, source[imax, jmax]))
    source = mwva.variables["spatial_sample_quality"][:][:]
    mwir_spatial_sample_quality = np.concatenate((mwir_spatial_sample_quality, source[imax, jmax]))
    source = lwva.variables["global_pcr_scores"][:][:]
    lwir_global_pcr_scores = np.concatenate((lwir_global_pcr_scores, source[imax, jmax]))
    source = lwva.variables["global_pcrs_quality"][:][:]
    lwir_global_pcrs_quality = np.concatenate((lwir_global_pcrs_quality, source[imax, jmax]))
    source = lwva.variables["spatial_sample_quality"][:][:]
    lwir_spatial_sample_quality = np.concatenate((lwir_spatial_sample_quality, source[imax, jmax]))
    source = mwva.variables["global_pc_scores"][:][:][:]
    mwir_global_pc_scores = np.concatenate((mwir_global_pc_scores, source[imax, jmax][:]))
    source = lwva.variables["global_pc_scores"][:][:][:]
    lwir_global_pc_scores = np.concatenate((lwir_global_pc_scores, source[imax, jmax][:]))
