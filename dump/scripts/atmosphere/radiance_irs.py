#!/usr/bin/env python3
# read mtg/irs netcdf files compute radiances and write to obsforge ioda file
...
def apply_hamming(x, nchan):
    hamming0 = np.float64(0.54)
    hamming1 = np.float64(0.23)
    y = x.copy()
    ncn = nchan - 1
    y[0] = (x[0] * hamming0 + x[1] * hamming1) / (hamming0 + hamming1)
    y[ncn] = (
        (x[ncn] * hamming0 + x[ncn - 1] * hamming1) / (hamming0 + hamming1))
    for icn in range(1, ncn):
        y[icn] = (
            x[icn - 1] * hamming1 + x[icn] * hamming0 + x[icn + 1] * hamming1)
    return y
...
reconst = os.path.join(
    parms, "RSP_OPE_BASEEV_MTS1+IRS_20230925000000_V1_out.h5")
yaml = os.path.join(yamls, "radiance_irs.yaml")

# --- read the channel data for lw and mw ir wave numbers

chan_lw = np.loadtxt(lwchans, dtype=int)
chan_lw = chan_lw[chan_lw != 0]
chns_lw = chan_lw.shape[0]
chan_mw = np.loadtxt(mwchans, dtype=int)
chan_mw = chan_mw[chan_mw != 0]
chns_mw = chan_mw.shape[0]

# --- read reconstruction means and operators for lw and mw ir channels

hd5file = Dataset(reconst)
means = hd5file['lwir'].variables['Mean'][:]
means_lw = means[chan_lw - 1]

means = hd5file['mwir'].variables['Mean']
means_mw = means[chan_mw - 1]
recop = hd5file['lwir'].variables['ReconstructionOperator']
recop_lw = recop[:, chan_lw - 1]
wnum_lw = recop.shape[1]
recop = hd5file['mwir'].variables['ReconstructionOperator']
recop_mw = recop[:, chan_mw - 1]
wnum_mw = recop.shape[1]
...
for filename in netcdf_files:
    try:
        ...
        sys.stderr.write(
            f"Skipping file '{filename}': failed to open as NetCDF: {e}\n")
        continue
...
for filename in file_keep:
    ...
    time = np.concatenate((time, scalar))
    scalar[:] = loca.variables["dwell_number"][:]
    dwell_number = np.concatenate((dwell_number, scalar))
    scalar[:] = loca.variables["stroke_direction"][:]
    stroke_direction = np.concatenate((stroke_direction, scalar))
    scalar[:] = mwva.variables["residual_energy"][:]
    mwir_residual_energy = np.concatenate((mwir_residual_energy, scalar))
    scalar[:] = lwva.variables["residual_energy"][:]
    lwir_residual_energy = np.concatenate((lwir_residual_energy, scalar))

    source = loca.variables["latitude"][:][:]
    latitude = np.concatenate((latitude, source[imax, jmax]))
    source = loca.variables["longitude"][:][:]
    longitude = np.concatenate((longitude, source[imax, jmax]))
    source = loca.variables["satellite_azimuth_angle"][:][:]
    satellite_azimuth_angle = np.concatenate(
        (satellite_azimuth_angle, source[imax, jmax]))
    source = loca.variables["satellite_zenith_angle"][:][:]
    satellite_zenith_angle = np.concatenate(
        (satellite_zenith_angle, source[imax, jmax]))
    source = loca.variables["solar_azimuth_angle"][:][:]
    solar_azimuth_angle = np.concatenate(
        (solar_azimuth_angle, source[imax, jmax]))
    source = loca.variables["solar_zenith_angle"][:][:]
    solar_zenith_angle = np.concatenate(
        (solar_zenith_angle, source[imax, jmax]))
    source = loca.variables["cloud_signal"][:][:]
    cloud_signal = np.concatenate((cloud_signal, source[imax, jmax]))
    source = loca.variables["cloud_fraction"][:][:]
    cloud_fraction = np.concatenate((cloud_fraction, source[imax, jmax]))
    source = mwva.variables["global_pcr_scores"][:][:]
    mwir_global_pcr_scores = np.concatenate(
        (mwir_global_pcr_scores, source[imax, jmax]))
    source = mwva.variables["global_pcrs_quality"][:][:]
    mwir_global_pcrs_quality = np.concatenate(
        (mwir_global_pcrs_quality, source[imax, jmax]))
    source = mwva.variables["spatial_sample_quality"][:][:]
    mwir_spatial_sample_quality = np.concatenate(
        (mwir_spatial_sample_quality, source[imax, jmax]))
    source = lwva.variables["global_pcr_scores"][:][:]
    lwir_global_pcr_scores = np.concatenate(
        (lwir_global_pcr_scores, source[imax, jmax]))
    source = lwva.variables["global_pcrs_quality"][:][:]
    lwir_global_pcrs_quality = np.concatenate(
        (lwir_global_pcrs_quality, source[imax, jmax]))
    source = lwva.variables["spatial_sample_quality"][:][:]
    lwir_spatial_sample_quality = np.concatenate(
        (lwir_spatial_sample_quality, source[imax, jmax]))
    source = mwva.variables["global_pc_scores"][:][:][:]
    mwir_global_pc_scores = np.concatenate(
        (mwir_global_pc_scores, source[imax, jmax][:]))
    source = lwva.variables["global_pc_scores"][:][:][:]
    lwir_global_pc_scores = np.concatenate(
        (lwir_global_pc_scores, source[imax, jmax][:]))
...
container.add(
    'mwir_global_pc_scores', mwir_global_pc_scores, ['*', '*/PSCORE'])
...
container.add(
    'mwir_spatial_sample_quality', mwir_spatial_sample_quality, ['*'])
container.add('mwir_residual_energy', mwir_residual_energy, ['*'])
container.add(
    'lwir_global_pc_scores', lwir_global_pc_scores, ['*', '*/PSCORE'])
...
container.add(
    'lwir_spatial_sample_quality', lwir_spatial_sample_quality, ['*'])
