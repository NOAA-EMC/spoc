...
MAPPING_PATH = map_path('bufr_marine_insitu_surface_trkob.yaml')


class MarineInsituSurfaceTrkobObsBuilder(MarineInsituObsBuilder):
    ...
    def make_obs(self, comm, input_path):
        ...
        container.apply_mask(temp_mask & saln_mask)

        self._add_preqc_var(container, "seaSurfaceTemperature")
        self._add_preqc_var(container, "salinity")
        self._add_error_var(container, "seaSurfaceTemperature", error=0.3)
        self._add_error_var(container, "salinity", error=1.0)

        return container

add_main_functions(MarineInsituSurfaceTrkobObsBuilder)
